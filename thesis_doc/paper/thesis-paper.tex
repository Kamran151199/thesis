\documentclass[12pt,bibliography=totoc,listof=totoc,BCOR=5mm,DIV=12,oneside,open=any]{scrbook}
...
% header and footer line - page number centered as required by the thesis checklist
...
\graphicspath{{./img/}{./Figures/}{./Figures/icons/}{./Figures/generated/}}

% Optional project-specific hyphenation file from the original template.
% It is not present in this repository, so no external hyphenation file is loaded.

% Glossary support is loaded because it is part of the thesis template.
% No glossary entries are defined in this manuscript, so glossary printing is omitted.
...
\frontmatter
    \begin{titlepage}
        ...
        \vspace{1.5cm}
        {\huge\bfseries Cross-Modal Generative Alignment for Document, Chart, and Image Reasoning with Explanation-Aware Vision--Language Models\par}
        \vspace{1.2cm}
        {\large Registered thesis topic:\par}
        \vspace{0.2cm}
    ...
    \vspace{2cm}
    \noindent Potsdam, 2026\hfill Komron Valijonov
    \clearpage
...
Chapter 2 provides the preliminaries required to understand the thesis, including vision--language models, cross-modal alignment, generative and contrastive supervision, explanation-aware learning, parameter-efficient fine-tuning, evidence sensitivity, and the evaluation metrics used later in the work. Chapter 3 reviews recent literature on vision--language model design, document-heavy inputs, chart reasoning, faithfulness, critique, and efficient adaptation. Chapter 4 describes the methodology, including the datasets, prompt families, model setup, training objectives, evaluation protocol, and experimental settings. Chapter 5 presents the results first and then discusses what those results mean in relation to the research questions, the domain differences, the masking analysis, model scale, and transfer behaviour. Chapter 6 concludes the thesis and gives directions for future work.


\chapter{Preliminaries}
This chapter introduces the technical ideas that are needed before the methodology and results can be read properly.
The thesis uses several terms that are common in recent vision--language research, but they can be easy to mix up.
For example, answer accuracy, explanation quality, and visual grounding are related, but they are not the same thing.
Likewise, contrastive alignment, generative fine-tuning, and explanation-aware training all try to connect image information with language, but they do it through different training signals.
The purpose of this chapter is therefore to define the basic concepts in the same way they are used later in the experiments.

\section{Vision--language models and visual question answering}
A vision--language model is a model that processes both visual input and text input.
In this thesis, the visual input can be a natural image, a chart, a scanned document, or a science-style diagram.
The text input is usually a question, and in some datasets it also includes answer choices.
The model is then expected to produce an answer, and in some settings it is also expected to produce a rationale before the answer.

Visual question answering is the task form used throughout this thesis.
The basic structure is simple: an image and a question are given, and the model returns an answer.
However, the type of reasoning required can vary a lot across datasets.
In a natural-image question, the answer might depend on recognising an object or a simple relationship between objects.
In a chart question, the answer may depend on reading an axis label, comparing two bars, or estimating a value.
In a document question, the answer may be a short text span hidden inside a scanned page.
In a science or commonsense question, the model may need to combine image information with answer choices and learned background knowledge.

This variation is one reason why this thesis does not rely on one dataset only.
A model can look strong on one benchmark and still fail on another benchmark that requires a different kind of visual evidence.
For example, a model that is good at object recognition may still struggle with small text in a document.
A model that can answer multiple-choice science questions may not transfer cleanly to open-ended chart or document questions.
Therefore, the thesis treats visual question answering as a family of related tasks rather than one single problem.

A second important point is that the answer format changes across datasets.
ScienceQA and A-OKVQA are used as multiple-choice datasets in the main rationale-supervised experiments.
DocVQA, ChartQA, and VQAv2 are treated as open-ended or short-answer tasks in the completed runs.
Because of this, the evaluation cannot use one universal metric for every dataset.
The thesis instead uses each dataset's native headline metric and explains the comparison within each domain.

\section{Cross-modal alignment}
Cross-modal alignment means connecting information from different modalities, such as images and language, so that they can support the same prediction.
In a vision--language model, the visual side must provide useful information to the language side.
If this connection is weak, the model may still produce fluent text, but the answer may be based on language patterns rather than on the image.
That is the central problem behind this thesis.

The word alignment can refer to several levels of the model.
At the representation level, the model may learn that an image and a matching text description should be close to each other in an embedding space.
At the generation level, the model may learn to use image features while producing an answer token by token.
At the reasoning level, the model may learn to produce an intermediate explanation before giving the final answer.
All three levels are relevant, but they do not measure the same behaviour.

A model can have useful image--text representations and still generate weak answers.
It can generate a correct answer without producing a useful explanation.
It can also produce a plausible explanation that is not the real reason for the answer.
This is why the thesis separates the training objective from the evaluation axis.
The objective tells us what signal the model receives during fine-tuning.
The evaluation tells us which behaviour actually changed after fine-tuning.

The comparison in this thesis is controlled because model design, data handling, and compute budget can otherwise hide the effect of the objective.
If one method uses a larger model, more training data, and a different prompt format, then a higher score cannot be cleanly attributed to the training objective.
For that reason, the experiments try to keep the pipeline fixed and change only the objective inside each comparison group.
This does not remove every limitation, but it makes the comparison more interpretable.

\section{Generative supervision}
Generative supervision trains the model to produce a target text sequence.
In the simplest answer-only case, the target contains only the answer.
In a rationale-generative case, the target contains a reasoning sentence or rationale followed by the final answer.
The model receives the image and the question as input, and the loss is computed on the target tokens.

This type of supervision fits naturally with instruction-tuned vision--language models.
The model is already built to generate text, so fine-tuning can adapt the output format to a specific task.
For example, the model can learn that ScienceQA answers should be selected from answer choices, or that DocVQA answers are usually short text spans.
Generative fine-tuning is therefore a practical baseline for many visual question answering tasks.

The weakness of pure generative supervision is that it does not by itself prove grounding.
A model can learn to generate the expected answer format without relying on the right visual region.
It may also learn common dataset patterns.
For example, in multiple-choice settings, some questions can be partly solved from the text and answer options.
In document or chart settings, some answers may be repeated in common positions or formats.
This does not make generative supervision useless, but it means the evaluation needs to check more than final answer accuracy.

In this thesis, generative supervision plays two roles.
First, it is used as a direct training objective for BLIP-2 and Qwen2-VL runs.
Second, it is used as a control for explanation-aware training.
This is important because explanation-aware training should not only be compared against a frozen zero-shot model.
It should also be compared against a simpler model that was fine-tuned to produce answers or rationale-plus-answer text without the same span-weighting strategy.

\section{Contrastive supervision}
Contrastive supervision trains the model to bring matching image and text representations closer together while pushing non-matching pairs farther apart.
In this thesis, the contrastive branch is tested with BLIP-2 because its Q-Former provides a natural place to build an image representation.
...
This objective is different from ordinary text generation.
Generative training asks the model to produce the next target token.
Contrastive training asks the model to organise representations so that matching image--answer pairs are easier to retrieve.
In principle, this can encourage stronger image--answer alignment.
In practice, the effect depends on the model, the feature extraction path, the projection head, the batch size, and the data.
That is why the thesis does not assume the contrastive branch is useful just because it adds another loss term.
It tests the branch directly against a BLIP-2 generative control.

The retrieval diagnostic is important here.
If the contrastive loss is doing what it is supposed to do, the correct answer should rank higher than mismatched answers when compared against the image representation.
Recall at one, recall at five, recall at ten, and mean reciprocal rank give a simple way to inspect that behaviour.
These metrics are not the final task score, but they help show whether the contrastive objective actually improved representation alignment.

A modest contrastive result does not mean contrastive learning is generally useless.
It only means that under the completed setup, with the chosen BLIP-2 configuration and capped ScienceQA run, the contrastive-enhanced arm does not produce a clearly stronger result than the generative control.
This distinction matters because the thesis is about the controlled effect of the objective under the available budget, not about dismissing an entire class of methods.

\section{Rationales and explanation-aware learning}
A rationale is a short explanation that connects the question, the visual input, and the final answer.
In datasets such as ScienceQA and A-OKVQA, gold rationales are available, which makes it possible to train the model to generate reasoning text before the answer.
This thesis uses those datasets to test whether rationale supervision changes answer accuracy and explanation quality.

Explanation-aware learning goes one step beyond simply generating a rationale and an answer in one sequence.
The target is split into two spans: the explanation span and the answer span.
The training loss can then weight these spans differently.
The coefficient alpha controls how much emphasis is placed on the answer tokens compared with the explanation tokens.
This allows the thesis to ask whether answer supervision and rationale supervision help each other or compete with each other.

The alpha sweep is an ablation.
If alpha is too low, the model may focus too much on explaining and not enough on selecting the correct answer.
If alpha is too high, the model may focus on the answer while the explanation receives little or no supervised signal.
The length-aware control is different.
It reflects the natural token-level cross-entropy balance that comes from the number of answer and explanation tokens.
This control is useful because fixed alpha values can otherwise give a misleading impression of what explanation-aware training is doing.
...
\section{Scope of the completed thesis}
...
% ---------------------------------------------------------------
\backmatter % no chapter numbering from here
