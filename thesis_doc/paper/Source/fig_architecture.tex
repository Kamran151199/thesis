...
\node[subtitle,below=1pt of title] {frozen visual backbone, trainable QLoRA adapters, and span-aware supervision};
...
\node[card,text width=4.6cm,minimum height=1.1cm,below=6mm of loss] (eval)
  {\textbf{Retained outputs}\\[-1pt]
   \scriptsize answer accuracy, rationale quality,\\ evidence-masking faithfulness};

% BLIP-2 reference lane
\node[card,text width=13.55cm,minimum height=0.85cm] (blip) at (7.8,-1.65)
  {\textbf{BLIP-2 reference path for alignment analyses.}
   \scriptsize Image $\rightarrow$ ViT $\rightarrow$ Q-Former learned queries $\rightarrow$ LLM;
   ITC / ITM / ITG pretraining exposes pooled image--text representations for contrastive comparisons.};
