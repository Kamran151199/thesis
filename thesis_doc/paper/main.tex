...
\begin{abstract}
A vision--language model (VLM) will read a chart, a scanned form, or a photograph and answer almost any question you put to it.
The harder question is whether it answered for the right reason.
An answer on its own does not settle that, and when the model adds an explanation, the explanation only counts if it ties back to what is actually in the image.
Charts and documents make this concrete: the supporting evidence is usually sitting somewhere in the input, which means it can be checked.
The trouble is that the field rarely isolates the training objective.
Contrastive, generative, and explanation-based methods tend to be compared across different models, datasets, and compute budgets at once, so it is never clear what the objective itself contributed.
We hold the pipeline fixed and change only the training signal.
Five settings are compared head to head rather than as unrelated systems: BLIP-2 generative fine-tuning, BLIP-2 contrastive-enhanced fine-tuning, and Qwen2-VL under answer-only, rationale-generative, and explanation-aware supervision.
Every run shares a resource-constrained single-seed protocol and, where possible, the same evaluation code, across ScienceQA, A-OKVQA, ChartQA, DocVQA, and a VQAv2 subset.
The picture that comes back is mixed.
On ScienceQA, Qwen2-VL-2B reaches 0.800 accuracy with answer-only fine-tuning, 0.780 with rationale-generative fine-tuning, 0.780 with the length-aware explanation control, and 0.765 with the fixed balanced explanation-aware setting.
On A-OKVQA the explanation-aware run reaches 0.795 and the rationale-generative control reaches 0.790, a difference too small to lean on under the capped run.
ChartQA, DocVQA, and the VQAv2 subset have no gold rationales, so those completed runs fall back to answer-only training; they lift the native headline metrics, but they do not test rationale supervision at all.
BLIP-2 barely moves: in the completed run, contrastive-enhanced training tops the generative baseline on ScienceQA only 0.480 to 0.475, and the retrieval diagnostics sit close to the frozen and random controls.
Scale is the exception.
Hold the explanation-aware objective fixed and Qwen2-VL-7B reaches 0.885 on ScienceQA where the 2B model reaches 0.765.
We read the masking analysis as evidence sensitivity, not as proof that the rationale text is faithful.
None of this adds up to a clean win for explanations: rationale supervision can help on a single-GPU budget, but only if the answer-only controls, the balance of the objective, model scale, transfer, and evidence sensitivity are each reported on their own.
\end{abstract}

\begin{graphicalabstract}
\centering
\includegraphics[width=0.95\linewidth]{Figures/icons/Abstract.pdf}
...
\begin{figure*}[!ht]
\centerline{\includegraphics[width=\linewidth]{./Figures/icons/Pipeline}}
...
\begin{figure*}[!ht]
\centerline{\includegraphics[height=0.74\textheight,keepaspectratio]{./Figures/icons/ModelArchitecture.pdf}}
