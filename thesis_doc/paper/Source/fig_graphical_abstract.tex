...
\node[title] (title) at (8.2,7.35)
  {Controlled explanation-aware VLM alignment study};
\node[subtitle] at (8.2,6.95)
  {fixed-budget comparison of generative, contrastive, and explanation-aware supervision};
...
\node[main,text width=4.45cm,minimum height=2.0cm] (model) at (7.55,4.55)
  {\textbf{Efficient VLM adaptation}\\[-1pt]
   \scriptsize Qwen2-VL + QLoRA\\
   \scriptsize BLIP-2 Q-Former\\[4pt]
   \scriptsize generative, contrastive,\\[-1pt]
   \scriptsize and explanation-aware signals};
...
\node[metric,text width=3.2cm,minimum height=1.05cm] (m1) at (4.0,1.85)
  {\textbf{Correctness}\\[-1pt]\scriptsize accuracy, ANLS,\\[-1pt]\scriptsize relaxed accuracy};
\node[metric,text width=3.2cm,minimum height=1.05cm] (m2) at (8.2,1.85)
  {\textbf{Rationale quality}\\[-1pt]\scriptsize ROUGE-L, BLEU\\[-1pt]\scriptsize for rationale runs};
\node[metric,text width=3.2cm,minimum height=1.05cm] (m3) at (12.4,1.85)
  {\textbf{Grounding}\\[-1pt]\scriptsize image masking,\\[-1pt]\scriptsize masking drift};
...
% Evaluation axes branch to all three metric boxes. The controlled-study banner
% below is experimental framing (a takeaway), not a data sink, so no arrows enter it.
\coordinate (evalhub) at ($(measure.south)+(0,-0.13)$);
\draw[guide] (measure.south) -- (evalhub);
\draw[guide,->] (evalhub) -| (m1.north);
\draw[guide,->] (evalhub) -- (m2.north);
\draw[guide,->] (evalhub) -| (m3.north);
